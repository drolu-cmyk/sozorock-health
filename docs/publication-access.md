...
The publication workflow separates public summaries from controlled PDF delivery and treats access reliability, identity confidence, and analytics as separate concerns.

## Visitor journey

1. A visitor opens a public publication summary. First-party source/referral context is retained for the publication journey so campaign information is not lost when the visitor moves to the separate access page.
2. The visitor requests access and gives publication-access consent. Publication-update consent is separate and optional.
3. The server validates the request, rejects obvious placeholder/gibberish input, validates the country and administrative area, applies hourly and daily abuse limits, and creates a 12-hour HTTP-only publication access session.
4. The visitor can immediately request the publication download. The server also attempts to send a single-use email verification link; delivery is not required for publication access. When used, verification confirms mailbox control, raises the record's confidence score, and creates a fresh 12-hour access session.
...
A public form cannot prove a person's legal identity, employer, or affiliation. The system therefore records evidence and confidence instead of treating every submitted field as verified fact.
...
- Reserved/fake email domains are rejected. Consumer, organizational, and known disposable email domains are classified for quality review; consumer email addresses remain valid publication-access addresses.
- Email verification proves mailbox control only. It does not prove the person's declared employer, location, or identity.
...
- Countries with a maintained structured administrative-area list use controlled state/province/territory choices.
...
- coarse network country/region when the hosting layer supplies those values.
...
Shared networks remain usable because no single network-level limit is the only decision signal. Obvious malformed/gibberish records fail validation, while lower-confidence but plausible readers can still access the public-interest publication.
...
Browser-originated funnel events carry the same first-touch source/referral and coarse device context so summary-page views, access starts, and completed requests can be compared by source and medium.
...
- Deploy `infrastructure/cloudformation/publication-access.yml` with the private publication storage, access table, and `PublicationIntelligence` index configured.
