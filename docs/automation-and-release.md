...
A public-site `Deploy` does not request an Evidence/Explore production release. That broader release is requested through a reviewed change to `release/explore-production.request` or an explicit Evidence activation dispatch for an exact `main` commit. Its production environment review, migration, rollback, and proof gates remain separate. Evidence proof statuses describe an actual Explore release, not a routine website deployment.
