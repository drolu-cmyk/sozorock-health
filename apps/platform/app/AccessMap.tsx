...
export default function AccessMap({records,selectedFips,onSelect}:{records:CountyAccessRecord[];selectedFips:string|null;onSelect:(fips:string)=>void}){
  ...
  return <div className="map-frame" data-testid="county-choropleth" onPointerLeave={()=>setTip(null)}><svg className="county-map" viewBox="0 0 975 610" role="img" aria-label="Interactive United States county access map. Tab to a highlighted county and press Enter for details."><path d={path(geometry)??""} className="county-base" aria-hidden="true"/>{counties.map((item)=><path key={item.fips} d={path(item.geometry)??""} fill={color(item.index)} className={`county-data ${selectedFips===item.fips?"selected":""}`} tabIndex={0} role="button" data-testid={`county-${item.fips}`} aria-label={`${item.county}, ${item.state}: access index ${item.index}`} onPointerMove={(event)=>{const box=event.currentTarget.ownerSVGElement?.getBoundingClientRect();if(box)setTip({x:event.clientX-box.left,y:event.clientY-box.top,county:item.county,state:item.state,index:item.index});}} onFocus={()=>setTip({x:50,y:40,county:item.county,state:item.state,index:item.index})} onBlur={()=>setTip(null)} onClick={()=>onSelect(item.fips)} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();onSelect(item.fips);}}}><title>{item.county}, {item.state} · Access index {item.index}</title></path>)}</svg>{tip&&<div className="map-tip" style={{left:`${Math.min(tip.x+12,72)}%`,top:`${Math.min(tip.y+12,72)}%`}} role="status"><strong>{tip.county}</strong><span>{tip.state}</span><b>{tip.index}<small>/100</small></b></div>}{!counties.length&&<p className="map-empty">No counties match these filters.</p>}</div>;
}
