type LinkedSource = { id: string; sourceId: string; releaseDate: string; retrievedAt: string; reviewedAt?: string };
...
export function currentContextSources<T extends LinkedSource>(linked: T[]): T[] {
  ...
  for (const source of [...linked].sort((a, b) => b.releaseDate.localeCompare(a.releaseDate)
    ...
    || (b.reviewedAt ?? "").localeCompare(a.reviewedAt ?? "") || a.id.localeCompare(b.id))) {
