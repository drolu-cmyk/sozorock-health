export const EXPLORE_OPENAPI_VERSION = "2026-08-09";
...
const sectionKeyParameter = { name: "sectionKey", in: "path", required: true, schema: { type: "string", pattern: "^[a-z][a-z0-9_-]{1,63}$" } } as const;
const errorResponse = {
...
export const exploreOpenApiDocument = {
  ...
  paths: {
    ...
    "/api/evidence/v1/voice/transcribe": { post: { tags: ["Voice"], summary: "Transcribe an intentionally recorded planning question without retaining raw audio.", requestBody: { required: true, content: { "multipart/form-data": { schema: { type: "object", required: ["geoid", "audio"], properties: { geoid: countyGeoid, audio: { type: "string", contentEncoding: "binary" } } } } } }, responses: { "200": { description: "Reviewable transcript." }, "400": errorResponse, "403": errorResponse, "413": errorResponse, "415": errorResponse, "422": errorResponse, "429": errorResponse, "503": errorResponse } } },
    "/api/evidence/v1/workspaces": { get: { tags: ["Workspace"], summary: "List authenticated workspaces.", security: [{ cognitoAccessToken: [] }], responses: { "200": { description: "Membership-scoped workspaces." } } }, post: { tags: ["Workspace"], ...jsonPost("Create a county workspace.", { type: "object" }, { type: "object" }, true, "201") } },
    ...
    "/api/evidence/v1/workspaces/{workspaceId}/sections/{sectionKey}": { parameters: [workspaceIdParameter, sectionKeyParameter], put: { tags: ["Workspace"], ...jsonPost("Create or update a versioned plan section with optimistic concurrency.", { type: "object", required: ["expectedVersion", "content"], properties: { expectedVersion: { type: "integer", minimum: 0 }, content: { type: "object" } } }, { type: "object" }, true) } },
    "/api/evidence/v1/workspaces/{workspaceId}/artifacts": { parameters: [workspaceIdParameter], post: { tags: ["Workspace"], ...jsonPost("Create an idempotent comment or review question, complete a review question, or review an agent suggestion.", {
      oneOf: [
        { type: "object", required: ["action", "sectionKey", "body"], properties: { action: { const: "comment" }, sectionKey: sectionKeyParameter.schema, body: { type: "string", minLength: 1, maxLength: 4000 } } },
        { type: "object", required: ["action", "sectionKey", "question"], properties: { action: { const: "review_question" }, sectionKey: sectionKeyParameter.schema, question: { type: "string", minLength: 3, maxLength: 2000 }, assignedTo: { type: ["string", "null"] }, isPublic: { type: "boolean" } } },
        { type: "object", required: ["action", "reviewQuestionId", "status"], properties: { action: { const: "complete_review_question" }, reviewQuestionId: { type: "string", format: "uuid" }, status: { type: "string", enum: ["answered", "closed"] } } },
        { type: "object", required: ["action", "suggestionId", "decision", "expectedSectionVersion"], properties: { action: { const: "review_suggestion" }, suggestionId: { type: "string", format: "uuid" }, decision: { type: "string", enum: ["accepted", "rejected"] }, expectedSectionVersion: { type: "integer", minimum: 0 } } },
      ],
    }, { type: "object" }, true, "201") } },
    ...
    "/api/evidence/v1/workspaces/{workspaceId}/invitations": { parameters: [workspaceIdParameter], post: { tags: ["Workspace"], ...jsonPost("Create an expiring role-scoped invitation as the workspace owner.", { type: "object", required: ["role", "access", "intendedPrincipalId"], properties: { role: { type: "string", enum: ["county_planner", "community_partner", "foundation_reviewer", "research_funder_viewer"] }, access: { type: "string", enum: ["contributor", "viewer"] }, intendedPrincipalId: { type: "string", minLength: 1 } } }, { type: "object" }, true, "201") } },
    "/api/evidence/v1/workspaces/{workspaceId}/fork": { parameters: [workspaceIdParameter], post: { tags: ["Workspace"], ...jsonPost("Create an idempotent active planning fork linked to the source workspace.", { type: "object", required: ["title"], properties: { title: { type: "string", minLength: 3, maxLength: 240 } } }, { type: "object" }, true, "201") } },
    "/api/evidence/v1/workspaces/{workspaceId}/handoff": { parameters: [workspaceIdParameter], post: { tags: ["Workspace"], ...jsonPost("Create an expiring workspace handoff for an approved participant role.", { type: "object", required: ["targetRole"], properties: { targetRole: { type: "string", enum: ["county_planner", "community_partner", "foundation_reviewer", "research_funder_viewer"] }, targetPrincipalId: { type: ["string", "null"] }, expiresInHours: { type: "integer", minimum: 1, maximum: 72 } } }, { type: "object" }, true, "201") } },
    "/api/evidence/v1/workspaces/{workspaceId}/share": { parameters: [workspaceIdParameter], get: { tags: ["Workspace"], summary: "List active share-link metadata for the workspace owner.", security: [{ cognitoAccessToken: [] }], responses: { "200": { description: "Active share-link metadata; bearer tokens are never returned." }, "403": errorResponse } }, post: { tags: ["Workspace"], ...jsonPost("Create an expiring public read-only share link.", { type: "object", properties: { scope: { type: "string", enum: ["read_only"], default: "read_only" }, expiresInHours: { type: "integer", minimum: 1, maximum: 168, default: 72 } } }, { type: "object" }, true, "201") }, delete: { tags: ["Workspace"], ...jsonPost("Revoke a public share link.", { type: "object", required: ["shareId"], properties: { shareId: { type: "string", format: "uuid" } } }, { type: "object" }, true) } },
