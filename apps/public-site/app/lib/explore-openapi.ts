...
export const exploreOpenApiDocument = {
  ...
  paths: {
    ...
    "/api/evidence/v1/workspaces/{workspaceId}": { get: { tags: ["Workspace"], summary: "Load a membership-scoped county plan.", security: [{ cognitoAccessToken: [] }], parameters: [{ name: "workspaceId", in: "path", required: true, schema: { type: "string", format: "uuid" } }], responses: { "200": { description: "Workspace plan." }, "403": errorResponse } } },
    "/api/evidence/v1/workspaces/{workspaceId}/events": { get: { tags: ["Workspace"], summary: "Read append-only events after a sequence.", security: [{ cognitoAccessToken: [] }], responses: { "200": { description: "Event stream." } } }, post: { tags: ["Workspace"], ...jsonPost("Append an authorized idempotent workspace event.", { type: "object" }, { type: "object" }, true) } },
    "/api/evidence/v1/workspaces/{workspaceId}/share": { get: { tags: ["Workspace"], summary: "List active share-link metadata for the workspace owner.", security: [{ cognitoAccessToken: [] }], responses: { "200": { description: "Active share-link metadata; bearer tokens are never returned." }, "403": errorResponse } }, post: { tags: ["Workspace"], ...jsonPost("Create an expiring public share link.", { type: "object" }, { type: "object" }, true) }, delete: { tags: ["Workspace"], ...jsonPost("Revoke a public share link.", { type: "object", required: ["shareId"], properties: { shareId: { type: "string", format: "uuid" } } }, { type: "object" }, true) } },
    "/api/evidence/v1/workspaces/{workspaceId}/audit": { get: { tags: ["Workspace"], summary: "Read append-only workspace and agent audit records as an owner or Foundation reviewer.", security: [{ cognitoAccessToken: [] }], responses: { "200": { description: "Authorized compliance audit view." }, "403": errorResponse } } },
    ...
    "/api/evidence/v1/workspaces/{workspaceId}/scenarios": { post: { tags: ["Workspace"], ...jsonPost("Create a versioned planning scenario owned by a human.", { type: "object" }, { type: "object" }, true) } },
    "/api/evidence/v1/heat-map": { post: { tags: ["Visuals"], ...jsonPost("Return compatible multi-county values and official boundaries.", { $ref: "#/components/schemas/CountySetRequest" }, { type: "object" }) } },
