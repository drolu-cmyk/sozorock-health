...
export async function POST(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    await createAccessRequest(publication.slug, input);
    ...
    console.error("publication-access-failed", { name: (error as { name?: string }).name ?? "UnknownError", slug: publication.slug });
