...
export default function PrivacyPage() {
  return (
    <LegalPage eyebrow="Public policy" title="Privacy notice" updated="July 13, 2026">
      <h2>Our approach</h2>
      ...
      <h2>Voice Access</h2>
      <p>Microphone access begins only after you review the voice notice and agree. On the website, OpenAI processes microphone audio during the live, non-clinical Voice Access session. OpenAI states that API data is not used to train its models by default. Unless different data controls are approved for the project, content may be retained in abuse-monitoring logs for up to 30 days under <a href="https://platform.openai.com/docs/guides/your-data" rel="noreferrer">OpenAI&apos;s API data-controls policy</a>. In the mobile app, speech recognition may use the speech service available on the Apple or Android device and may be processed under Apple&apos;s, Google&apos;s, or the device provider&apos;s terms. SozoRock Health does not send a device-speech transcript to its access-request API or store it with a request. Do not share medical records or protected health information. You can stop a session at any time, and text or tap options remain available. SozoRock Health does not intentionally record or retain raw microphone audio.</p>
