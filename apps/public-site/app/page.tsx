  ...
  return (
    <main>
      <header className="nav-shell">
        <a className="wordmark" href="#top" aria-label="SozoRock Health home"><span>Sozo</span><b>R</b><span>ock</span><sup>®</sup><i> Health</i></a>
      ...
      <section id="counties" className="cbcap">
        <div className="section-intro"><p className="eyebrow">CB-CAP</p><h2>See the conditions around access. Act before they become a crisis.</h2><p>Internal teams begin with national visibility. Interested counties receive approved access to protected, aggregated intelligence.</p></div>
        <div className="dashboard">
          ...
          <div className="dash-main"><div className="filters"><button>United States</button><button>State</button><button>County</button><button>ZIP code</button><button>Time range</button><button className="apply">Apply filters</button><button className="export">Export CSV</button><button className="export">Download report</button></div><div className="dash-grid"><div className="funnel"><h3>Access funnel <em>Controlled</em></h3>{[100,65,42,24,15].map((value, index)=><div key={value}><span style={{width:`${value}%`}} /> <small>{["Community presence","Aware of options","Attempted access","Connected to resource","Needs follow-up"][index]}</small></div>)}</div><div className="map-panel"><h3>Access index map</h3><div className="county-map" /></div><div className="trend"><h3>Access trend <em>Controlled</em></h3><svg viewBox="0 0 500 150" aria-label="Access trend chart"><path d="M0 130 C40 120 70 90 110 104 S175 85 220 93 S290 55 330 70 S390 38 430 50 S470 28 500 18" fill="none" stroke="currentColor" strokeWidth="5" /></svg></div><div className="barriers"><h3>Top barriers</h3>{["Transportation","Cost","Information gap","Digital access","Language"].map((name,index)=><div key={name}><small>{name}</small><span style={{width:`${90-index*13}%`}} /></div>)}</div></div><footer>All data is aggregated and de-identified. Small populations are suppressed. <a href="#about">Read methodology →</a></footer></div>
        </div>
      </section>
      ...
      <section id="partner" className="partner"><p className="eyebrow">Partner with SozoRock</p><h2>Build access with your community.</h2><p>For counties, providers, libraries, community organizations, universities, and funders.</p><a href="mailto:contact@sozorockfoundation.org">Start a conversation</a><small>Contact form with consent, abuse protection, and rate limits is being activated for launch.</small></section>
