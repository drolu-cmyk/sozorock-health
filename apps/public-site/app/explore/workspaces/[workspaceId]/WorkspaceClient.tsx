...
export function WorkspaceClient({ workspaceId }: { workspaceId: string }) {
  ...
  const loadOwnerControls = useCallback(async (nextPlan: Plan) => {
    if (nextPlan.actor.access === "owner") {
    ...
    if (nextPlan.actor.access === "owner" || nextPlan.actor.role === "foundation_reviewer") {
  ...
  const writable = plan.actor.access !== "viewer";
  const canAudit = plan.actor.access === "owner" || plan.actor.role === "foundation_reviewer";
  ...
  return <main className={styles.shell}>
    ...
    <div className={styles.workspaceGrid}>
      <section id="plan" className={styles.workspacePanel}><header><h2>Named plan sections</h2><span>Optimistic version checks protect concurrent edits.</span></header>{plan.sections.length ? plan.sections.map((section) => <SectionEditor key={section.sectionKey} section={section} writable={writable} onSave={saveSection} />) : <p>No plan section has been started.</p>}</section>
